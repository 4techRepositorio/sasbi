# 4Pro_BI — 5 frentes paralelas e prompts por agente

**Papel:** gestão técnica (derivado de `docs/AGENTS.md`, `docs/VISION.md`, `docs/ARCHITECTURE.md`, `tickets/`).  
**Escopo desta partição:** tickets **000–010** (Marco A laboratório + SaaS fechável com quotas); **011+** são evoluções do produto, salvo decisão explícita de puxar para esta esteira.

---

## 1. Cinco frentes independentes (áreas de repositório disjuntas)

| # | Frente | Áreas do repo (donos) | Tickets típicos (fase base) | Agente (`docs/AGENTS.md`) |
|---|--------|------------------------|----------------------------|---------------------------|
...
### Onda 3 — Fecho da fase base (000–010)
...
Objetivos:
1. Manter docs/ARCHITECTURE.md alinhado ao produto 4Pro_BI (blocos, domínios, experiência unificada sem marcas OSS na UX).
...
3. Definir/estabilizar contratos em packages/contracts (DTOs) para endpoints críticos da fase base, com impacto documentado.
...
Restrições: não implementes parsing, fila Celery nem routers dedicados a upload/ingestions/datasets — isso é Backend Data. Contratos em packages/contracts: alinhar com Architect (PR dedicado).
...
```text
Contexto: apps/web Angular — portal multitenant 4Pro_BI. docs/wireframes/validation-*.md, .cursor/rules/03-frontend.mdc. Tickets (fase base): fluxos 001–005 e páginas 006–009 + shell.
...
```text
Contexto: projeto 4Pro_BI — tickets 000–010 (fase base). docs/CHECKLISTS/, pytest em apps/api, scripts de smoke. docs/AGENTS.md papel QA Reviewer.
