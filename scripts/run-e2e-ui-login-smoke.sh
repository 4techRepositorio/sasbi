#!/usr/bin/env bash
# Smoke UI: login → /app (conta sem MFA). Requer front em PLAYWRIGHT_BASE_URL e credenciais.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
# shellcheck source=lib/e2e-env.sh
source "$ROOT/scripts/lib/e2e-env.sh"

e2e_load_dotenv "$ROOT"

if [[ -z "${E2E_USER_EMAIL:-}" || -z "${E2E_USER_PASSWORD:-}" ]]; then
  echo "Erro: defina E2E_USER_EMAIL e E2E_USER_PASSWORD (ficheiro e2e/.env.e2e ou export)." >&2
  exit 1
fi

export PLAYWRIGHT_BASE_URL="${PLAYWRIGHT_BASE_URL:-http://127.0.0.1:4200}"

echo "==> E2E — smoke login"
echo "    PLAYWRIGHT_BASE_URL=$PLAYWRIGHT_BASE_URL"

cd "$ROOT/e2e"
npm ci
e2e_install_browsers_if_requested "$ROOT"
npx playwright test tests/smoke-login.spec.ts "$@"
